...
class Bing(MethodView):
    def get(self):
        ...
        url = 'http://api.microsofttranslator.com/v2/ajax.svc/TranslateArray2?appId=%22TQj2aNVgceliQSTJi6g05wUp7B5sXi0FrXrOwuZnhQeI*%22&texts=[%22'\
          + urllib.quote(query) +'%22]&from=%22zh-chs%22&to=%22en%22&oncomplete=_mstc6&onerror=_mste6&loc=en&ctr=&'
...
class Best2(MethodView):
    ...
    def get(self):
        ...
        for i in range(0, 6):
            ...
            Parser_Feature.append(str(float(correct)/float(len(s_dic))))
        f_tran.close()
        ...
        # 生成phrase_table
        f_pt = open('./files/phrase_table','w')
        f_pt.writelines('_1_ ||| ' + nrcsent + ' ||| '  + str(Parser_Feature[0])  +' ||| |||\n')
        f_pt.writelines('_1_ ||| ' + pbtsent + ' ||| '  + str(Parser_Feature[1]) +' ||| |||\n')
        f_pt.writelines('_1_ ||| ' + amlsent + ' ||| ' + str(Parser_Feature[2]) +' ||| |||\n')
        f_pt.writelines('_1_ ||| ' + jxsent + ' ||| ' + str(Parser_Feature[3]) + ' ||| |||\n')
        f_pt.writelines('_1_ ||| ' + shsent + ' ||| ' + str(Parser_Feature[4]) + ' ||| |||\n')
        f_pt.writelines('_1_ ||| ' + srisent + ' ||| ' + str(Parser_Feature[5]) + ' ||| |||\n')
        f_pt.close()
        # 利用online.ini翻译， 将翻译出来的结果保存
        # 生成input文件 _1_
        f_in = open('./files/input.file','w')
...
class Dependency(MethodView):
    def get(self):
        ...
        depend = [{},{},{},{},{},{}]
        engine_list = ['NRC','RWTH_PBT','RWTH_PBT_AML','RWTH_PBT_JX','RWTH_PBT_SH','SRI_HPBT']
        for i in xrange(6):
        ...
        for i in xrange(6):
            ...
            length = len(depend[i]['correct_dependency'])
            depend[i]['correct_dependency'].append(' ')
            depend[i]['correct_dependency'].append('Preserved: ' + str(length))
        #'''              
...
class TxtDB(MethodView):
    def get(self):
        filename_CN = 'csent-test-common.seg'        
        filename_EN = 'test-ref0OSRoot.test-ref0'

        f_CN = open(filename_CN, 'rb')
        f_EN = open(filename_EN, 'rb')
        db_list_CN = f_CN.readlines()  # 读取文本文件到list, db_list=[行1，行2，..., 行n]
        db_list_EN = f_EN.readlines() 
        f_CN.close()
        f_EN.close()

        index = 1
        txtdb = []
        for row in db_list_CN:
            if row.strip() != '':
                txtdb.append({'index': str(index), 'row': str(index) + ":" + row, 'en_row': db_list_EN[index-1]})  # 以row做index
            index += 1
