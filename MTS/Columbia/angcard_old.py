...
class Bing(MethodView):
    def get(self):
        ...
        url = url = 'http://api.microsofttranslator.com/v2/ajax.svc/TranslateArray2?appId=%22TruyoYsbMsa7NUYItoJWYb5A8is_CyWtqqJDL4A6q_L3I9ub9dOWzD459aWO4njCf%22&texts=[%22'\
          + urllib.quote(query) +'%22]&from=%22zh-chs%22&to=%22en%22&oncomplete=_mstc2&onerror=_mste2&loc=en&ctr=&'
        ...
        translate_result = trans_res['TranslatedText']
        # 返回爬虫结果         
        return jsonify(success=True, translate_result=translate_result)
...
class Best(MethodView):
    ...
    def get(self):
        ...
        subprocess.call('/local/kz2203/moses/mosesdecoder/bin/moses -f online.ini < ./files/input.file > ./files/translated.file',shell = True)
...
class Best2(MethodView):
    ...
    def get(self):
        ...
        subprocess.call('/local/kz2203/perfect/stanford-parser/lexparser.sh '\
                        + file_in + ' > ' + file_out, shell = True)

        file_in = './files/engine.file'
        file_out = './files/engine_p.file'
        subprocess.call('/local/kz2203/perfect/stanford-parser/lexparser-en.sh '\
                        + file_in + ' > ' + file_out, shell = True)
        p1 = re.compile('\(ROOT')
