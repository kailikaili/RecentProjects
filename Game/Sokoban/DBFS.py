...
def Solve_BFS(puzzle, max_rowlen, line_num, wall_data, state_data, px, py, \
    ...
    open_Queue = deque([(state_data, "", px, py)])
    ...
    while open_Queue:
        cur, csol, x, y = open_Queue.popleft()
        for di in dirs:
            temp = cur
            dx, dy = di[0], di[1]

            if temp[(y + dy) * l_max_rowlen + x + dx] == '*':
                temp = Push(x, y, dx, dy, temp, wall_data, l_max_rowlen)
                if temp:
                    nodesTotal += 1
                    if temp not in visited:
                        if deadlock(x, y, dx, dy, temp, wall_data, l_max_rowlen, line_num):
                            ...
                            if is_Solved(temp, wall_data):
                                ...
                                return True
                            open_Queue.append((temp, csol + di[2].upper(), x+dx, y+dy))
                    else:
                        nodesPrev += 1
            else:
                ...
                if temp not in visited:
                    ...
                    if is_Solved(temp, wall_data):
                        ...
                        printstats([nodesTotal_flag, nodesPrev_flag, nodesRem_flag, nodesExpl_flag, runTime_flag], \
                                   [nodesTotal, nodesPrev, nodesRem, nodesExpl, round(end_time - start_time, 4)])
                        return True
                    open_Queue.append((temp, csol + di[2], x + dx, y + dy))
                else:
                    nodesPrev += 1
...
def DBFS(game_file):
    ...
    Solve_BFS(puzzle, max_rowlen, line_num, wall_data, state_data, px, py, 1, 1, 1, 1, 1)
